...
This script should take about ~15 minutes to run now
"""
...
if __name__ == "__main__":
    ...
    logger.info("loading and reformattting datasets...this will take ~6 minutes")
    ...
    green_matches = []
    for skill_chunk in list_chunks(extracted_skill_embeddings_skills, batch_size):
        top_green_skills = sm.get_green_skill_matches(
            extracted_skill_list=skill_chunk,
            similarities=similarities,
            green_skills_taxonomy=green_skills_taxonomy,
            skill_threshold=gm.skill_threshold,
        )
        green_matches.extend(top_green_skills)

    all_extracted_green_skills_dict = {sk[0]: (sk[0], sk[1]) for sk in green_matches}
