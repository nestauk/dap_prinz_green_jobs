...
skill_metrics_name = "ojo_all_skills_green_measures_skill_metrics.parquet"
green_skills_exploded_name = "ojo_all_skills_green_measures_exploded_green.parquet"
all_skills_exploded_name = "ojo_all_skills_exploded.parquet"

salary_loc = "s3://prinz-green-jobs/outputs/data/ojo_application/deduplicated_sample/20241114/latest_update_20241114_salaries.parquet"
location_loc = "s3://prinz-green-jobs/outputs/data/ojo_application/deduplicated_sample/20241114/latest_update_20241114_locations.parquet"
...
if __name__ == "__main__":
    job_id_col = "job_id"
    # Import and combine all the measures for all the job adverts

    logger.info("Loading skills data")
    green_skills_metrics = pd.read_parquet(
        f"s3://prinz-green-jobs/outputs/data/ojo_application/extracted_green_measures/{analysis_config['skills_date_stamp']}/{skill_metrics_name}",
    )

    green_skills = pd.read_parquet(
        f"s3://prinz-green-jobs/outputs/data/ojo_application/extracted_green_measures/{analysis_config['skills_date_stamp']}/{green_skills_exploded_name}",
    )
    ...
    salary_information = pd.read_parquet(salary_loc)
    locations_information = pd.read_parquet(location_loc)
    ...
    all_skills_data = pd.read_parquet(
        f"s3://prinz-green-jobs/outputs/data/ojo_application/extracted_green_measures/{analysis_config['skills_date_stamp']}/{all_skills_exploded_name}"
    )
...
# del sys.modules["dap_prinz_green_jobs.analysis.ojo_analysis.gje_formatting"]
...
for itl_col in ["itl_3_code", "itl_2_code", "itl_1_code"]:
    ...
    df.to_csv(
        os.path.join(root_s3_dir, f"prop_green_skills_per_sic_{itl_col}_{today}.csv"),
        index=False,
    )
